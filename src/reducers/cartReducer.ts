...
export function cartReducer(state: CartState, action: CartAction): CartState {
    ...
    switch (action.type) {
        case 'INCREMENT':
            existing = state.items.find(item => item.fruitId === action.payload.fruitId)
            if (existing) {
                return {
                    items: state.items.map(item => item.fruitId === action.payload.fruitId ? { ...item, quantity: item.quantity + 1 } : item)
                }
            }
